...
type Tab = "living-page" | "ats-resume" | "theme" | "preview";
...
function isTab(value: string | null): value is Tab {
  return value === "living-page" || value === "ats-resume" || value === "theme" || value === "preview";
}
...
function buildAtsStatusMessage(
  ...
  if (isAtsOutOfSync(atsReview, data)) {
    return "Your ATS resume is out of sync with the living page. Reset it from the living page when you want to resync.";
  }

  if (hasApprovedAtsResume(atsReview)) {
    return "ATS PDF ready on the live page.";
  }

  if (atsReview?.approvedResumeData) {
    return "ATS draft saved, but it still needs edits before it becomes a one-page PDF.";
  }

  return "ATS resume not generated yet. Use the ATS tab to build it.";
}
...
export default function EditPage() {
  ...
  const [tab, setTab] = useState<Tab>("living-page");
  ...
  const isDirty = hasLivingChanges || hasAtsChanges;
  ...
  }, [atsData, atsTargeting, data, isDirty, pageConfig, saveDraft, tab, themeId]);
  ...
  const buildPersistedAtsReview = useCallback(
    async (
      ...
      return finalizeApprovedAtsResume(
        inherited,
        nextAtsData,
        approvedSourceFingerprint ?? atsReview?.approvedSourceFingerprint ?? buildAtsRelevantFingerprint(data ?? nextAtsData),
      );
    },
  ...
  const runAtsReview = useCallback(
    ...
    [atsData, atsReview, atsTargeting, page?.id, page?.raw_resume, router, trackOptimizeEvent],
  );
  ...
  const handleSaveClick = useCallback(async () => {
    ...
    let nextPageConfig = pageConfig;
    if (hasAtsChanges || !atsReview) {
      const nextReview = await buildPersistedAtsReview(atsData, atsTargeting);
      if (!nextReview) {
        return;
      }
      nextPageConfig = { ...pageConfig, ats: nextReview };
    }

    if (!hasLivingChanges && !hasAtsChanges && themeId === (page.theme_id as ThemeId)) {
      return;
    }

    await persistPage(data, atsData, nextPageConfig, themeId, atsTargeting);
  }, [
    atsData,
    atsReview,
    atsTargeting,
    buildPersistedAtsReview,
    data,
    hasAtsChanges,
    hasLivingChanges,
    page,
  ...
  const handleUseAutoOptimizedVersion = useCallback(() => {
    if (!atsReview || !atsData) {
      return;
    }

    const nextReview = approveCandidateAtsResume(atsReview, atsReview.approvedSourceFingerprint ?? null);
    setAtsData(resolveEditableAtsResumeData(nextReview, atsData));
    setPageConfig((prev) => ({ ...prev, ats: nextReview }));
    setSuccess(
      hasApprovedAtsResume(nextReview)
        ? "Auto-optimized ATS version loaded. Save when you're ready."
        : "Auto-optimized ATS draft loaded. Save when you're ready.",
    );
    setTimeout(() => setSuccess(""), 3000);
  ...
  }, [atsData, atsReview, page?.id, trackOptimizeEvent]);
  ...
  const handleResetFromLivingPage = useCallback(async () => {
    ...
    const nextReview = await buildPersistedAtsReview(nextAtsData, nextTargeting, nextFingerprint);
    if (!nextReview) {
      return;
    }
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 md:px-10">
      <div className="mb-5 flex flex-col justify-between gap-3 sm:mb-6 sm:flex-row sm:items-end sm:gap-4">
        <div>
          ...
          <h1 className="mt-2 font-heading text-2xl font-bold text-[#F0F4FF] sm:text-3xl">{data.name}</h1>
        </div>
      ...
      <div className="mb-6 flex gap-1 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-1">
        {TAB_OPTIONS.map((option) => (
          <button
            ...
            className="flex-1 rounded-lg px-3 py-2 text-[11px] font-medium uppercase tracking-[0.14em] transition-all sm:px-4 sm:py-2.5 sm:text-xs"
            style={{
              background: tab === option.id ? "rgba(59,130,246,0.12)" : "transparent",
              color: tab === option.id ? "#93C5FD" : "rgba(240,244,255,0.45)",
              borderColor: tab === option.id ? "rgba(59,130,246,0.25)" : "transparent",
            }}
          >
            {option.label}
          </button>
        ...
        <div className="space-y-5">
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6">
              {atsOutOfSync
                ? "Your living page changed after this ATS resume was last synced. Reset from the living page when you want to pull the latest source content in."
          ...
          <AtsReviewPanel
            ...
            body="Use the ATS suggestions when they help, or keep editing the ATS version manually. Save whenever the ATS draft looks right."
          />
        </div>
      ) : null}

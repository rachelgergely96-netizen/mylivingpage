  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 sm:px-6 py-6 sm:py-10 md:px-10">
      <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4">
        ...
        <Link
          href="/create"
          className="gold-pill self-start sm:self-auto px-5 py-2.5 sm:px-6 sm:py-3 text-xs font-semibold uppercase tracking-[0.16em] transition-all duration-300 ease-soft hover:shadow-[0_10px_36px_rgba(212,166,84,0.35)]"
        >
          Create New Page
        </Link>
      </div>
      ...
      {!list.length ? (
        ...
        <section className="grid gap-3">
          {list.map((page) => (
            ...
            >
              <div>
                <p className="font-heading text-lg sm:text-2xl text-[#F5F0EB]">{page.resume_data?.name ?? "Untitled"}</p>
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                <Link
                  href={`/${page.slug}`}
                  className="rounded-full border border-[rgba(212,166,84,0.35)] px-3 py-1.5 sm:px-4 sm:py-2 text-xs uppercase tracking-[0.14em] text-[#D4A654] hover:text-[#F0D48A]"
                >
                  View
                </Link>
                <Link
                  href={`/dashboard/edit/${page.id}`}
                  className="rounded-full border border-[rgba(255,255,255,0.15)] px-3 py-1.5 sm:px-4 sm:py-2 text-xs uppercase tracking-[0.14em] text-[rgba(245,240,235,0.6)] hover:border-[rgba(212,166,84,0.35)] hover:text-[#F0D48A]"
                >
                  Edit
                </Link>
                <Link
                ...
                <button
                  type="button"
                  disabled
                  className="rounded-full border border-[rgba(255,255,255,0.15)] px-3 py-1.5 sm:px-4 sm:py-2 text-xs uppercase tracking-[0.14em] text-[rgba(245,240,235,0.4)]"
                >
                  Archive
                </button>
              </div>

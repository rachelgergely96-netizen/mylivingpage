      ...
      <div className="mx-auto max-w-4xl">
        ...
        <header className="mb-5 sm:mb-6 flex items-start justify-between gap-3 sm:gap-4">
          <div className="min-w-0">
            <h1 className={`${headingSize} font-heading font-bold text-[#F5F0EB] drop-shadow-[0_2px_16px_rgba(0,0,0,0.5)] leading-tight`}>
              {data.name}
            </h1>
            <p className="mt-1 text-[10px] sm:text-xs uppercase tracking-[0.2em] text-[#D4A654]">{data.headline}</p>
            {data.location ? <p className="mt-1 text-xs sm:text-sm text-[rgba(245,240,235,0.4)]">{data.location}</p> : null}
            {hasContact ? (
              <div className="mt-2 flex flex-wrap items-center gap-2 sm:gap-3">
                {data.email ? (
                  <a href={`mailto:${data.email}`} className="pointer-events-auto flex items-center gap-1.5 py-1 text-[10px] sm:text-xs text-[rgba(245,240,235,0.5)] transition-colors hover:text-[#F0D48A]">
                    <svg className="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg>
                    <span className="truncate">{data.email}</span>
                  </a>
                ) : null}
                {data.linkedin ? (
                  <a href={data.linkedin.startsWith("http") ? data.linkedin : `https://${data.linkedin}`} target="_blank" rel="noopener noreferrer" className="pointer-events-auto flex items-center gap-1.5 py-1 text-[10px] sm:text-xs text-[rgba(245,240,235,0.5)] transition-colors hover:text-[#F0D48A]">
                ...
                {data.github ? (
                  <a href={data.github.startsWith("http") ? data.github : `https://github.com/${data.github}`} target="_blank" rel="noopener noreferrer" className="pointer-events-auto flex items-center gap-1.5 py-1 text-[10px] sm:text-xs text-[rgba(245,240,235,0.5)] transition-colors hover:text-[#F0D48A]">
                ...
                {data.website ? (
                  <a href={data.website.startsWith("http") ? data.website : `https://${data.website}`} target="_blank" rel="noopener noreferrer" className="pointer-events-auto flex items-center gap-1.5 py-1 text-[10px] sm:text-xs text-[rgba(245,240,235,0.5)] transition-colors hover:text-[#F0D48A]">
          ...
          {data.avatar_url ? (
            <img
              src={data.avatar_url}
              alt={data.name}
              className="h-11 w-11 sm:h-14 sm:w-14 shrink-0 rounded-full object-cover ring-2 ring-[#D4A654] shadow-[0_0_28px_rgba(212,166,84,0.3)]"
            />
          ) : (
            <div className="flex h-11 w-11 sm:h-14 sm:w-14 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#D4A654] to-[#E8845C] font-heading text-xl sm:text-2xl font-bold text-[#1A0A2E] shadow-[0_0_28px_rgba(212,166,84,0.3)]">
        ...
        {/* ── Summary ─────────────────────────────────────────────── */}
        {data.summary ? <p className={`${summarySize} mb-5 sm:mb-6 leading-6 sm:leading-7 text-[rgba(245,240,235,0.6)]`}>{data.summary}</p> : null}
        ...
        {data.stats?.length ? (
          <section className="mb-5 sm:mb-6 grid grid-cols-2 gap-2 md:gap-3 sm:grid-cols-3 md:grid-cols-4">
            {data.stats.slice(0, 4).map((stat) => (
              ...
              >
                <div className="font-mono text-base sm:text-xl text-[#F0D48A]">{stat.value}</div>
                <div className="mt-0.5 sm:mt-1 text-[9px] sm:text-[10px] uppercase tracking-[0.18em] text-[rgba(245,240,235,0.45)]">{stat.label}</div>
              </article>
            ))}
          </section>
        ) : null}
                ...
                <div key={group.category}>
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {group.items.map((skill) => (
                      <span
                        key={skill}
                        className="rounded-full border border-[rgba(212,166,84,0.25)] bg-[rgba(212,166,84,0.08)] px-2.5 py-0.5 text-[11px] sm:px-3 sm:py-1 sm:text-xs text-[#F0D48A]"
                      >
                        {skill}
                      </span>
                    ))}
          ...
          <section>
            ...
            <div className="space-y-2">
              {certs.map((cert) => (
                ...
                >
                  <div className="flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-lg bg-[rgba(212,166,84,0.1)]">

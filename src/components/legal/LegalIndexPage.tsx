    ...
    <div className="min-h-screen">
      <header className="border-b border-[rgba(255,255,255,0.08)] bg-[rgba(26,10,46,0.72)] backdrop-blur-xl">
        <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4 sm:h-20 sm:px-6">
          ...
          <p className="text-xs uppercase tracking-[0.2em] text-[rgba(245,240,235,0.55)]">Legal Center</p>
        </div>
      </header>

      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        <section className="glass-card rounded-3xl p-6 sm:p-10">
          <p className="text-xs uppercase tracking-[0.2em] text-[#D4A654]">Last updated: {"{{EFFECTIVE_DATE}}"}</p>
          <h1 className="mt-3 font-heading text-3xl font-bold text-[#F5F0EB] sm:text-5xl">Legal and Policies</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[rgba(245,240,235,0.68)] sm:text-base">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-2xl border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.02)] p-4 transition-all hover:-translate-y-0.5 hover:border-[rgba(212,166,84,0.32)]"
              >
                <p className="font-heading text-2xl text-[#F5F0EB]">{link.label}</p>
                <p className="mt-2 text-sm leading-6 text-[rgba(245,240,235,0.58)]">{link.description}</p>
              </Link>
            ))}
          </div>
        </section>

  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(10,22,40,0.85)] p-4">
      ...
      <div className="mb-3 flex items-center justify-between">
        <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#3B82F6]">&#x1F4CA; This Week</span>
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div>
          <p className="font-mono text-[9px] text-[rgba(240,244,255,0.3)]">Views</p>
          <p className="font-mono text-sm text-[#F0F4FF]">{isPro ? views.toLocaleString() : "???"}</p>
        </div>
        <div>
          <p className="font-mono text-[9px] text-[rgba(240,244,255,0.3)]">Unique</p>
          <p className="font-mono text-sm text-[#F0F4FF]">{isPro ? unique.toLocaleString() : "???"}</p>
        </div>
        <div>
          <p className="font-mono text-[9px] text-[rgba(240,244,255,0.3)]">Avg. Time</p>
          <p className="font-mono text-sm text-[#F0F4FF]">{isPro ? MOCK_ANALYTICS.avgTime : "???"}</p>
        </div>
        <div>
          <p className="font-mono text-[9px] text-[rgba(240,244,255,0.3)]">Top Section</p>
          <p className="font-mono text-sm text-[#F0F4FF]">{isPro ? MOCK_ANALYTICS.topSection : "—"}</p>
        </div>
      </div>
      ...
      >
        {MOCK_ANALYTICS.referrals.map((r) => (
          <span key={r.label} className="rounded-md bg-[rgba(59,130,246,0.1)] px-2 py-0.5 font-mono text-[9px] text-[rgba(59,130,246,0.7)]">
            {r.label} {r.pct}
          </span>
        ))}
      </div>

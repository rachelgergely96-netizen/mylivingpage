...
export const renderTerracotta: ThemeRenderer = (
  ...
  for (let i=0;i<12;i++){
    ...
    const strokeFacingRim=(threshold:number,width:number,alpha:number,lightness:number)=>{
      ctx.beginPath();
      let drawingRim=false;
      pts.forEach(([px,py,ang])=>{
        if (Math.cos(ang-sourceAngle)>threshold){
      ...
      ctx.lineWidth=width;
      ctx.strokeStyle=`hsla(${hue+18},70%,${lightness}%,${alpha})`;
      ctx.stroke();
    };
    ctx.save(); ctx.clip();
    strokeFacingRim(0.94,2.2,0.10,60);
    strokeFacingRim(0.985,1,0.12,64);
    ctx.restore();
    // slip rings inside
